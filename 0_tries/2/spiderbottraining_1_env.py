...
from isaaclab.utils.math import sample_uniform,euler_xyz_from_quat,wrap_to_pi
...
class Spiderbottraining1Env(DirectRLEnv):
 ...
 """
    ...
    def _get_observations(self) -> dict:
        obs = torch.cat(
            (
                ...
                self.robot.data.joint_pos[:, self._joint_dof_idx], #previous joint positions
                self.actions, #8 elements
    ...
    def _get_rewards(self) -> torch.Tensor:
        # 1. forward velocity — just go forward
        vx = self.robot.data.root_lin_vel_w[:, 0]
        rew_forward = torch.clamp(vx, min=0.0) * self.cfg.rew_scale_vel

        # 2. don't fall over
        roll, pitch, _ = euler_xyz_from_quat(self.robot.data.root_quat_w)
        tilt = torch.sqrt(roll**2 + pitch**2)
        rew_orientation = self.cfg.rew_scale_orientation * tilt  # negative scale

        return torch.clamp(rew_forward + rew_orientation, -5.0, 5.0)

    def _get_dones(self) -> tuple[torch.Tensor, torch.Tensor]:
        time_out = self.episode_length_buf >= self.max_episode_length - 1

        # use roll/pitch instead of root_z since origin is on bottom face
        roll, pitch, _ = euler_xyz_from_quat(self.robot.data.root_quat_w)
        fallen = (torch.abs(roll) > torch.pi / 3) | (torch.abs(pitch) > torch.pi / 3)
